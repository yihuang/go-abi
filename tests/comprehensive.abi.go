...
func EncodeUint256Array3(value [3]*abi.Uint256, buf []byte) (int, error) {
	// Encode fixed-size array with static elements
...
func EncodeUint256SliceSlice(value [][]*abi.Uint256, buf []byte) (int, error) {
	// Encode length
	binary.BigEndian.PutUint64(buf[24:32], uint64(len(value)))
	buf = buf[32:]
...
func SizeUint256SliceSlice(value [][]*abi.Uint256) int {
	size := 32 + 32*len(value) // length + offset pointers for dynamic elements
	for _, elem := range value {
...
func DecodeUint256Array3(data []byte) ([3]*abi.Uint256, int, error) {
	// Decode fixed-size array with static elements
	var (
...
func DecodeUint256SliceSlice(data []byte) ([][]*abi.Uint256, int, error) {
	// Decode length
	if len(data) < 32 {
		return nil, 0, io.ErrUnexpectedEOF
	}
	...
	// Decode elements with dynamic types
	result := make([][]*abi.Uint256, length)
	dynamicOffset := length * 32
	for i := 0; i < length; i++ {
		tmp, err := abi.DecodeSize(data[offset:])
		if err != nil {
...
func PackedEncodeUint256Array3(value [3]*abi.Uint256, buf []byte) (int, error) {
	if len(buf) < 96 {
		return 0, io.ErrShortBuffer
	}
	// Encode fixed-size array elements sequentially (no padding)
...
func PackedDecodeUint256Array3(data []byte) ([3]*abi.Uint256, int, error) {
	if len(data) < 96 {
