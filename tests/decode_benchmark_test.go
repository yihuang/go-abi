...
func BenchmarkGoABI_DecodeMemoryAllocations_ComplexDynamicTuples(b *testing.B) {
	args := createComplexDynamicTuplesData()
	encoded, err := args.Encode()
	if err != nil {
		b.Fatal(err)
...
func BenchmarkGoEthereum_DecodeMemoryAllocations_ComplexDynamicTuples(b *testing.B) {
	args := createComplexDynamicTuplesData()
	encoded, err := ComprehensiveTestABIDef.Pack("testComplexDynamicTuples", args.Users)
	if err != nil {
		b.Fatal(err)
