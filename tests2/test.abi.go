...
func (e TransferEventIndexed) EncodeTopics() ([]common.Hash, error) {
	...
	topics = append(topics, TransferEventTopic)
	{
		...
		if _, err := _TestEncodeAddress(e.From, hash[:]); err != nil {
			return nil, err
		}
		topics = append(topics, hash)
	}
	{
		// To
		var hash common.Hash
		if _, err := _TestEncodeAddress(e.To, hash[:]); err != nil {
			return nil, err
		}
		topics = append(topics, hash)
	}
...
func (e *TransferEventIndexed) DecodeTopics(topics []common.Hash) error {
	...
	var err error
	e.From, _, err = _TestDecodeAddress(topics[1][:])
	if err != nil {
		return err
	}
	e.To, _, err = _TestDecodeAddress(topics[2][:])
	if err != nil {
		return err
	}
	return nil
...
func (value TransferEventData) EncodeTo(buf []byte) (int, error) {
	...
	// Field Value: uint256
	if _, err := _TestEncodeUint256(value.Value, buf[0:]); err != nil {
		return 0, err
	}
...
func (t *TransferEventData) Decode(data []byte) (int, error) {
	...
	t.Value, _, err = _TestDecodeUint256(data[0:])
	if err != nil {
		return 0, err
	}
	return dynamicOffset, nil
